...
?><div <?php wc_product_class( $classes, $product ); ?>>
	...
	<div class="product-small box <?php echo flatsome_product_box_class(); ?>">
		<div class="box-image">
			...
			<div class="icon_1">
			    <?php if( get_field('icon_1-1') ): ?>
                    <img src="<?php the_field('icon_1-1'); ?>" />
                <?php endif; ?>
			</div>
			<div class="icon-gift">
			    <?php if( get_field('icon_gift') ): ?>
                    <img src="<?php the_field('icon_gift'); ?>" />
                <?php endif; ?>
			</div>
		</div>
		...
		<div class="box-text <?php echo flatsome_product_box_text_class(); ?>">
			...
			?>
			<div class="ant-progress" style="display: none">
            	<?php
            	$progress = get_field('number_sale');
            	if( !$progress ) {
            		$progress = 10;
            	}
            	?>
            	<style type="text/css">
            		.post-<?php echo esc_html( $product->id ); ?> .progress_bar {
            			width: <?php echo esc_html( $progress ); ?>%;
            		}
            	</style>
            	<span class="progress_bar"></span>
            	<span>còn <?php echo esc_html( $progress ); ?> sản phẩm</span>
            </div>
		</div>
	</div>
