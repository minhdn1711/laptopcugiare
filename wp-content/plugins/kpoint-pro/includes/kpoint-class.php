...
class KPoint{
	...
	public static function get_display_balance($amount, $include_label = true){
		global $kpoint_settings;
		$number_decimal = $kpoint_settings['number_decimal'];
        ...
        if($is_round) return number_format($amount, 0, ',', '.') . ' ' . $label;;
        if($number_decimal == 0) $number_decimal = 1;
		return number_format($amount, $number_decimal, ',', '.') . ' ' . $label;
	}
		...
		if(isset($logs) && count($logs) > 0){
			...
			<table class="kp-logs">
				...
				<?php foreach ($logs as $log) : 
					if(is_numeric($log->ref)){
						$link = wc_get_endpoint_url( 'view-order', $log->ref, wc_get_page_permalink( 'myaccount' ) );
						$log->ref = "<a href='$link'>#{$log->ref}</a>";
					}
					?>
					<tr>
						...
						<td><?php echo date("d/m/Y H:i", strtotime($log->time)); ?></td>
